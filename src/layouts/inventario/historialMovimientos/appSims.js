...
const AppHistorialS = () => {
    ...
    const tienePermisoEspecifico = permissions.includes(PERMISOS.ACCEDER_HISTORIAL_SIMS_ESPECIFICO)
    const tieneIdEmpleado = user?.empleado_id || 0

    useEffect(() => {
        if (tienePermisoEspecifico) {
            dispatch(getHistorialM({
                tipo_producto_nombre: [TIPO_PRODUCTO.SIM_DISTRIBUCION, TIPO_PRODUCTO.SIM_PORTABILIDAD],
                empleado: tieneIdEmpleado,
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            Cell: ({ cell, row }) => {
                let localDestino = cell.getValue()
                let empleadoDestino = row.original.destino_empleado
                return (
                    <>
                        {empleadoDestino ? "" : localDestino ? localDestino : "Vendido"}
                    </>
                )
            },
        },
        {
            header: "Empleado Destino",
            accessorKey: 'destino_empleado.nombre_completo'
        },
        {
            header: 'Producto',
        ...
        },
        {
            ...
            Cell: UnidadesColumn, // Renderiza el botón y el modal
            muiTableBodyCellProps: {
                sx: {
                    textAlign: "center",
                },
