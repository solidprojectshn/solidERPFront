...
import { getExistenciasEmpleados} from "../../../../redux/inventario/verExistencias/actions";
...
const AppEGeneral = () => {
    ...
    const tieneEmpleadoId = user?.empleado_id || 0
    ...
    const opcionesEmpleados = useMemo(() => {
        ...
        return Array.from(
            new Set(existencias.map((producto) => producto.empleado.nombre_completo))
        ).map((empleado) => ({ value: empleado, label: empleado }));
    }, [existencias]);

    // Filtrar productos según el empleado seleccionado
    const productosFiltrados = useMemo(() => {
        if (!existencias) return [];
        if (!empleadoSeleccionado) return existencias;
        return existencias.filter(
            (producto) => producto.empleado.nombre_completo === empleadoSeleccionado.value
        );
    }, [empleadoSeleccionado, existencias]);
    ...
    useEffect(() => {
        ...
        existencias.forEach(({ empleado, stock }) => {
            if (!empleado?.id_empleado || stock <= 0) return;

            // Determinar mayor stock
            if (!mayor || stock > mayor.stock) {
                mayor = { nombre: empleado.nombre_completo, stock };
            }

            // Determinar menor stock
            if (!menor || stock < menor.stock) {
                menor = { nombre: empleado.nombre_completo, stock };
            }
        });
    ...
    useEffect(() => {
        if (tienePermisoEspecifico) {
            dispatch(getExistenciasEmpleados({ empleado: tieneEmpleadoId }));
        }
        else {
            dispatch(getExistenciasEmpleados());
        }
