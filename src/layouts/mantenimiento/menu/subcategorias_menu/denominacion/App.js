...
// api
import { getDenominacion, deleteDenominacion } from "../../../../../redux/mantenimientos/denominacion/actions";
...
// import complementos
import DenominacionForm from "./Components/DenominacionForm";
...
const app = () => {
  ...
  const {
    ...
    denominaciones,
    denominacionError,
  } = useSelector((store) => store.denominacion);
  ...
  useEffect(() => {
    dispatch(getDenominacion());
    console.log("Datos de las denominaciones:", denominaciones);
  ...
  const handleDelete = (data, index) => {
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          data.estado = false;
          dispatch(deleteDenominacion(data, data.id_denominacion, index));
        }
      });
  };
  ...
  return (
    <DashboardLayout>
      <Fondo>
        ...
        <BoxButton>
          {denominacionError && <Alert severity="error">{denominacionError}</Alert>}
        ...
        <DenominacionForm initialData={editItem} isEdit={isEdit} setShowModal={setShowModal} showModal={showModal} />
        <BoxTabla>
