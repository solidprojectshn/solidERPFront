...
// api
import { getRangoFactura, deleteRangoFactura } from "../../../../../redux/mantenimientos/rango_factura/actions";
...
const app = () => {
  ...
  const {
    ...
    rangoFacturas,
    rangoFacturaError,
  } = useSelector((store) => store.rangoFactura);
  ...
  useEffect(() => {
    ...
    console.log("Datos de los rangos:", rangoFacturas);
  }, []);
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          ...
          data.estado = false;
          data.local_id = data.local.id_local
          dispatch(deleteRangoFactura(data, data.id_rango_factura, index));
        }
      });
  };
  ...
  const columns = useMemo(() => [
    ...
    { accessorKey: 'local.nombre', header: 'Nombre Del Local' },
    { accessorKey: 'numero_inicio', header: 'Numero Inicio' },
    { accessorKey: 'numero_final', header: 'Numero Finalización' },
    { accessorKey: 'cai', header: 'CAI' },
  ...
  return (
    <DashboardLayout>
      <Fondo>
        ...
        <BoxButton>
          {rangoFacturaError && <Alert severity="error">{rangoFacturaError}</Alert>}
