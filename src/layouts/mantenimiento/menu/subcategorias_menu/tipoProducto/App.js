...
// api
import { getProductType, deleteProductType } from "../../../../../redux/mantenimientos/tipoProducto/actions";
...
// import complementos
import DialogForm from "./Components/TipoProductoForm";
import { TIPO_PRODUCTO } from "../../../../../services/constantes";
...
const app = () => {
  ...
  useEffect(() => {
    ...
    console.log("Datos de tipos de producto:", productTypes);
  }, [])
  ...
  const handleDelete = (data, index) => {
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          data.estado = false;
          dispatch(deleteProductType(data, data.id_tipo_producto, index));
        }
      });
  };
  ...
  const columns = useMemo(() => [
    {
      header: 'ID',
      accessorKey: 'id_tipo_producto',
    },
    { header: 'Nombre', accessorKey: 'nombre' },
    { header: 'Descripción', accessorKey: 'descripcion' },
    {
      header: '¿Posee unidades únicas?',
      accessorKey: 'unidades_unicas',
      size: 300,
      Cell: ({ cell }) => {
    ...
    },
    {
      ...
      Cell: ({ row }) => {
        const noEditables = [
          ...
          TIPO_PRODUCTO.NO_SERIALIZADOS,
          TIPO_PRODUCTO.SIM_DISTRIBUCION,
          TIPO_PRODUCTO.SIM_PORTABILIDAD
        ];
    
        // Verifica si el nombre del tipo producto está en los tipos permitidos
        const isInNoEditables = noEditables.includes(row.original.nombre);
  ...
  return (
    <DashboardLayout>
      <Fondo>
        ...
        <DialogForm initialData={editItem} isEdit={isEdit} setShowModal={setShowModal} showModal={showModal} />
        <BoxTabla>
