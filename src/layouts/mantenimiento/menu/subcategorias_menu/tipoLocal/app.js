...
// api
import {getTipoLocal, deleteTipoLocal} from "../../../../../redux/mantenimientos/tipoLocal/actions";
...
// import complementos
import TipoLocalForm from "../tipoLocal/Components/TipoLocalForm";
...
const app = () => {
  ...
  const {
    ...
    tipoLocales,
    tipoLocalError,
  } = useSelector((store) => store.tipoLocal);
  ...
  useEffect(() => {
    ...
    console.log("Datos de tipo de locales:", tipoLocales);
  }, []);
  ...
  const handleDelete = (data, index) => {
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          data.estado = false;
          dispatch(deleteTipoLocal(data, data.id_tipo_local, index));
        }
      });
  };
  ...
  const columns = useMemo(() => [
    ...
    { accessorKey: 'nombre', header: 'Nombre' },
    {
      ...
      Cell: ({ row }) => {
        const noEditables = [
          TIPO_LOCAL.BODEGA_PRINCIPAL,
          TIPO_LOCAL.PUNTO_ESTACIONARIO,
          TIPO_LOCAL.TIENDA
        ];
    
        // Verifica si el nombre del tipo local está en los tipos permitidos
  ...
  return (
    <DashboardLayout>
      <Fondo>
        ...
        <TipoLocalForm initialData={editItem} isEdit={isEdit} setShowModal={setShowModal} showModal={showModal} />
        <BoxTabla>
