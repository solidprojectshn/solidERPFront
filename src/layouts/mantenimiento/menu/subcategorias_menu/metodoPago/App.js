...
// api
import { getMetodoPago, deleteMetodoPago } from "../../../../../redux/mantenimientos/metodopago/actions";
...
// import complementos
import MetodoPForm from "./Components/MetodoPagoForm";
...
const app = () => {
  ...
  useEffect(() => {
    ...
    console.log("Datos de los metodos disponibles:", metodosPago);
  }, []);
  ...
  const handleDelete = (data, index) => {
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          data.estado = false;
          dispatch(deleteMetodoPago(data, data.id_metodo_pago, index));
        }
      });
  };
    ...
    { accessorKey: 'nombre', header: 'Nombre' },
    {
      ...
      Cell: ({ row }) => {
        const noEditables = [
          ...
          TIPO_METODO_PAGO.TARJETA_CREDITO
        ];

        // Verifica si el nombre del metodo de pago está en los tipos permitidos
  ...
  return (
    <DashboardLayout>
      <Fondo>
        ...
        <BoxButton>
          {metodoPagoError && <Alert severity="error">{metodoPagoError}</Alert>}
        ...
        <MetodoPForm initialData={editItem} isEdit={isEdit} setShowModal={setShowModal} showModal={showModal} />
        <BoxTabla>
