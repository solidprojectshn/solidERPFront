...
// api
import { getRol, deleteRol } from "../../../../../redux/mantenimientos/roles/actions";
...
import { ROLES } from "../../../../../services/constantes";
...
const app = () => {
  ...
  const handlePermisos = (data) => {
    navigate(`/mantenimiento/permisos/${data.id}`)
  ...
  const handleDelete = (data, index) => {
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          data.estado = false;
          dispatch(deleteRol(data, data.id, index));
        }
      });
  };
  ...
  const columns = useMemo(() => [
...

    {
      ...
      Cell: ({ row }) => {
        ...
        const isInNoEditables = noEditables.includes(row.original.name);
  ...
  return (
    <DashboardLayout>
      <Fondo>
        ...
        <RolForm initialData={editItem} isEdit={isEdit} setShowModal={setShowModal} showModal={showModal} />
