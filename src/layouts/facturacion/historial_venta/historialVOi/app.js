...
const HistorialVOI = () => {
    ...
    useEffect(() => {
        if(tienePermisoEspecifico){
            dispatch(getVenta({mes: selectedMes,tipo_venta: TIPO_VENTA.OI, local: tieneLocal}));
        } else{
            dispatch(getVenta({mes: selectedMes,tipo_venta: TIPO_VENTA.OI}));
        }
    ...
    const goToDetail = (id) => {
        navigate(`/historial/Venta/detalleOi/${id}`);
    }
