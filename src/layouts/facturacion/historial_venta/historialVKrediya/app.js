...
const BtnGenerarExcel = styled(Button)(({ theme }) => ({
    width: '99%',
    margin: '1%',
    height: '45%',
...
const HistorialVDursan = () => {
    ...
    useEffect(() => {
        if(tienePermisoEspecifico){
            dispatch(getVenta({mes: selectedMes,tipo_venta: TIPO_VENTA.KREDIYA, local: tieneLocal}));
        } else{
            dispatch(getVenta({mes: selectedMes,tipo_venta: TIPO_VENTA.KREDIYA}));
        }
    ...
    const goToDetail = (id) => {
        navigate(`/historial/Venta/detalleKrediya/${id}`);
    }
    ...
    const generateExcel = (ventas) => {
        ...
        const worksheet = XLSX.utils.json_to_sheet(
            ventas.map(item => ({
                ...
                'Fecha': moment(item.fecha_venta).format('DD-MM-YYYY HH:mm:ss'),
                'Subtotal (HNL)': (Number(item.total) / 1.15).toFixed(2),
                'ISV (HNL)': ((Number(item.total) / 1.15) * 0.15).toFixed(2),
                'Total (HNL)': Number(item.total).toFixed(2),
            }))
        );
    ...
    const generatePDF = () => {
        ...
        ventas.forEach((venta, index) => {
            ...
            doc.text(`DIRECCIÓN:${empresa.rtn}`, 15,32);
                ...
                <div>
                    <Fondo>
                        <SUno>
                            ...
                            <BtnGenerarExcel 
                                onClick={() => generateExcel(ventas)} 
                                disabled={loadingVenta || ventas.length === 0}
                            >
                                Generar Excel
                            </BtnGenerarExcel>
                        </SUno>
...
export default HistorialVDursan;