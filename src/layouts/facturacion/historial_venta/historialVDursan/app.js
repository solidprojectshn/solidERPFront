...
const HistorialVDursan = () => {
    ...
    useEffect(() => {
        if(tienePermisoEspecifico){
            dispatch(getVenta({ mes: selectedMes, tipo_venta: TIPO_VENTA.DURSAN , local: tieneLocal}));
        } else{
            dispatch(getVenta({ mes: selectedMes, tipo_venta: TIPO_VENTA.DURSAN }));
        }
        ...
        <DashboardLayout>
            ...
            {loadingVenta ? (
                ...
                <div>
                    <Fondo>
                        <SUno>
                            <BtnGenerar onClick={generatePDF} disabled={loadingVenta}>Generar PDF</BtnGenerar>
                            <BtnGenerarExcel 
