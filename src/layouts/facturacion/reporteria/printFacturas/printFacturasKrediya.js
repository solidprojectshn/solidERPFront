...
const PrintFactKrediya = () => {
    ...
    useEffect(() => {
        if(tienePermisoEspecifico){
            dispatch(getVenta({ tipo_venta: TIPO_VENTA.KREDIYA, local: tieneLocal}));
        } else{
            dispatch(getVenta({ tipo_venta: TIPO_VENTA.KREDIYA}));
        }
    }, [dispatch]);
...
export default PrintFactKrediya;