...
const DetallePage = () => {
    ...
    const generatePDF = () => {
        ...
        // Encabezado
        doc.setFontSize(9);
        doc.text('INVERSIONES DURSAN', 15, 24);
        ...
        const tableData = data.map((item) => [
            ...
            //precio del producto
            ((item.precio_venta) || 0).toLocaleString('es-HN', {
                style: 'currency',
                currency: 'HNL',
            }),
            ...
            (((item.precio_venta) * item.cantidad) || 0).toLocaleString('es-HN', {
                style: 'currency',
                currency: 'HNL',
            }),
        ]);
        ...
        // subtotal
        const subtotF = data.reduce((acc, item) => acc + ((item.precio_venta) * item.cantidad || 0), 0);
        ...
        autoTable(doc, {
            startY: 65,
            head: [['CANTIDAD', 'DESCRIPCION', 'PRECIO UNITARIO', 'DESCUENTOS Y REBAJAS OTORGADOS', 'TOTAL']],
            body: tableData,
