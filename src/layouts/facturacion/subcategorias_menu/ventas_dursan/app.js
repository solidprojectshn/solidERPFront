...
import { numberToWordsWithDecimals } from "layouts/facturacion/historial_venta/detalle/helpers";
...
const VentasDursan = () => {
    ...
    const printReceipt = (datosFactura) => {
        ...
        const fecha = new Date(datosFactura.fecha_venta).toLocaleDateString();
        ...
        const totalEnLetras = numberToWordsWithDecimals(datosFactura.total)
        ...
        const data = [
            ...
            `           Subtotal:       L.${parseFloat(datosFactura.total / 1.15).toFixed(2)}\n`, // Ejemplo ISV fijo
            `                ISV:       L.${parseFloat((datosFactura.total / 1.15) * 0.15).toFixed(2)}\n`, // Cambiar si es dinámico
            `             **Total:      L.${datosFactura.total}\n`,
    ...
    useEffect(() => {
        if (user) {
            ...
            dispatch(getCliente())
            dispatch(getTipoVenta({ tipo_venta: TIPO_VENTA.DURSAN }))
        }
    ...
    const total = dataDetalle.reduce((acc, producto) => {
            ...
            <DashboardLayout>
                ...
                {loadingLocales || loadingMetodosPago || loadingClientes || loadingNumFactura || loadingTipoVenta || loadingInfoEmpresa ? (
                    ...
                    <div >
                        ...
                        <BoxDellFactura>
                            <BoxDetalle>
                                {(Number(sumaMontosMetodoPagoTotal) < Number(total)) && (
                                    <Alert className="my-2" severity="error"> Faltan {Number(total - sumaMontosMetodoPagoTotal).toLocaleString('es-HN', { style: 'currency', currency: 'HNL' })} para llegar al monto total. </Alert>
                                )}
                                {(Number(sumaMontosMetodoPagoTotal) > Number(total)) && (
                                    <Alert severity="error"> Sobran {Number(sumaMontosMetodoPagoTotal - total).toLocaleString('es-HN', { style: 'currency', currency: 'HNL' })} del monto total. </Alert>
                                )}
                                <Box sx={{ padding: '2%' }}>
                                    <h5 className="font-bold p-2 my-2 bg-yellow-50">Pago Artículos</h5>
                                    {(Number(sumaMontosMetodoPago) < Number(total - montoEsperadoRecarga)) && (
                                        <Alert className="my-2" severity="error"> Faltan {Number((total - montoEsperadoRecarga) - sumaMontosMetodoPagoTotal).toLocaleString('es-HN', { style: 'currency', currency: 'HNL' })} para llegar al monto de los artículos. </Alert>
                                    )}
                                    {console.log(sumaMontosMetodoPago, total, montoEsperadoRecarga ,"articulos")}
                                    {(Number(sumaMontosMetodoPago) > Number(total - montoEsperadoRecarga)) && (
                                        <Alert severity="error"> Sobran {Number(sumaMontosMetodoPago - (total - montoEsperadoRecarga)).toLocaleString('es-HN', { style: 'currency', currency: 'HNL' })} del monto de los artículos. </Alert>
                                    )}
                                ...
                                <BoxTotales>
                                    ...
                                    <CarsDetalleT>
                                        ...
                                        <TextField
                                            variant='outlined'
                                            placeholder=""
                                            value={total.toLocaleString('es-HN', { style: 'currency', currency: 'HNL' })}
                                            inputProps={{
                                                style: {
                                                    color: '#000000',
                                                    fontSize: 'small',
