...
export const getHistorialM = (filters) => async (dispatch) => {
  try {
    ...
    dispatch({
      ...
      payload: {
        historialM: resp.data.results,
        historialMLength: resp.data.total_count,
      },
    });
    dispatch({
...
export const getHistorialMById = (id) => async (dispatch) => {
  try {
    dispatch({
      type: actions.LOADING_HISTORIAL_MOVIMIENTO_BY_ID,
      payload: true,
    });
    const resp = await historialmovServices.getHistorialByID(id);
    dispatch({
      type: actions.GET_HISTORIAL_MOVIMIENTO_BY_ID,
      payload: resp.data,
    });
    dispatch({
      type: actions.HISTORIAL_MOVIMIENTO_BY_ID_FAIL,
      payload: "",
    });
  } catch (ex) {
    let message = null;
